...
public class DefaultChatModelObservationConvention implements ChatModelObservationConvention {
	...
	private static final KeyValue REQUEST_MODEL_NONE = KeyValue.of(REQUEST_MODEL_KEY_NAME, KeyValue.NONE_VALUE);

	private static final KeyValue RESPONSE_MODEL_NONE = KeyValue.of(RESPONSE_MODEL_KEY_NAME, KeyValue.NONE_VALUE);
	// @formatter:on
	...
	public String getContextualName(ChatModelObservationContext context) {
		...
		return resolveRequestModelName(context)
			.map(modelName -> "%s %s".formatted(context.getOperationMetadata().operationType(), modelName))
	...
	private KeyValue keyValueOf(KeyName keyName, Object value) {
